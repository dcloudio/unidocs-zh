...
#### 按量计费每日资源阈值设置@aliyun-postpay-quota

按量计费服务空间提供每日各项资源阈值上限的设置，通过该项设置，可以实现类似`包年包月`套餐的模式，也可以更好的防止资源被恶意刷量。

资源阈值可在服务空间详情进行设置，该设置实时生效。每项资源指标的`数据更新延迟时间`不同，当数据更新且某项资源用量超过阈值时，则该项服务便会停服。
...
**注意**
- 阈值设置针对的是每日用量，当日用量超阈值上限便会停服，受`数据更新延迟时间`影响，在停服时资源用量可能会超出所设阈值
- 阈值设置实时生效，如果设置的阈值已超当时的用量，则会实时停服。每小时最多可操作三次
