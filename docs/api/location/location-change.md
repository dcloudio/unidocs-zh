...
|longitude|Number|The longitude, the range is -180~180, the negative number means the west longitude. ||
|speed|Number|速度 (m/s)||
...
Start monitoring real-time geographic location information change events, and receive real-time geographic location information when the MiniApp enters the front-end and back-end.
...
Turn off monitoring for real-time location changes, and stop message reception in both the front and backend.
...
|latitude|number|Latitude, the range is [-90, 90], a negative number means southern latitude| Kuaishou MiniApp|
|longitude|number|经度，范围 [-180, 180]，负数表示西经|快手小程序|
|longitude|number|Longitude, range [-180, 180], negative number means west longitude| Kuaishou MiniApp|
|speed|number|速度 (m/s)|快手小程序|
|speed|number|Speed(m/s)| Kuaishou MiniApp|
|accuracy|number|位置的精确度|快手小程序|
|accuracy|number|Location accuracy| Kuaishou MiniApp|
|altitude|number|高度 (m)|快手小程序|
|altitude|number|Altitude(m)| Kuaishou MiniApp|
|verticalAccuracy|number|垂直精度 (m)（Android 无法获取，返回 0）|快手小程序|
|verticalAccuracy|number|Vertical Accuracy (m) (Android cannot get it, returns 0) | Kuaishou MiniApp|
