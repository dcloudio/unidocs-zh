...
|mergeVirtualHostAttributes|Boolean|合并组件（由 vue 组件编译而成的小程序组件）[虚拟节点](/tutorial/vue-api.md#%E5%85%B6%E4%BB%96%E9%85%8D%E7%BD%AE)外层属性，目前仅支持 id（v4.42）、style（v3.5.1）、class（v3.5.1）以及 v-show 指令生成的 hidden（v4.41） 属性|
