...
> 由于阿里云云存储CDN域名未进行服务空间区分，可能存在域名被封禁时影响使用的情况，如遇到部分地区或部分网络无法访问阿里云请及时向我们反馈。阿里云后续会提供隔离方案。另外阿里云还提供了一个备用云存储CDN域名：vkceyugu-backup.cdn.bspapp.com，如有需求可以将文件url的域名替换为备用域名来使用
...
`数据万象`是腾讯云的一个按量计费产品，月初生成上个月的账单，并从账号余额中扣除相关费用。故若在腾讯云中使用图片缩放、图片水印等功能，首先需保证账号余额充足，否则可能会触发**账号级停服**风险！具体而言：
- 企业认证用户：在uniCloud控制台缴纳保证金，充值足够余额即可
- 个人认证用户：发送邮件到service@dcloud.io，申请开通按量计费服务，DCloud审核资料后，将该账号加入白名单；个人认证用户再在uniCloud控制台缴纳保证金，充值足够余额即可。申请邮件内需要提供以下信息
...
**注意：使用腾讯云的数据万象时，需保证uniCloud账号已缴纳保证金、且余额充足，否则可能触发账号级停服风险！**
