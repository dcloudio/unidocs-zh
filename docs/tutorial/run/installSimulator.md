...
#### 云端打包支持 arm 架构的自定义基座  
> HBuilderX5.21版本 uni-app x 蒸汽模式（Vapor）支持打包 arm 架构模拟器自定义基座  

1. HBuilderX 中云端打包界面选择 `制作自定义调试基座` 并选中 `模拟器（arm64）`  

![](https://web-ext-storage.dcloud.net.cn/doc/uniapp/armsimulator-package.png)
