...
99% of the applications need to develop functions such as user registration, login, sending SMS verification code, encrypted password storage, password modification, token management, etc., which are required from the front end to the back end.
...
5. Right-click the cloud function that needs to use `uni-id` and select `Manage public module dependencies` to add `uni-id` to the cloud function. After adding the dependency, you need to re-upload the cloud function
...
Calling the checkToken interface when the token expires in 5 minutes will return the new token and the expiration time of the new token (the validity time of the new token is also 2 hours), and the front end needs to actively save this new token.
...
uni-id encapsulates a rich API for the role permission module, such as: obtaining user roles, obtaining all permissions under a certain role, etc. For details, refer to: [Role permission API](uniCloud/uni-id.md?id=rbac-api ).
...
2. **Package** and **use** the custom base (note that you must fill in the WeChat appid in the manifest.json before making the custom base), [custom base usage instructions](https:// ask.dcloud.net.cn/article/35115)
...
Since `uni-id 3.0.0`, it is supported to cache the user's role permissions in the token. This function is enabled by default, and the needPermission parameter of each login interface is no longer valid. If you need to close it, please configure `"removePermissionAndRoleFromToken": true` in config.
...
It can be seen that the old version token (generated when removePermissionAndRoleFromToken is true) needs to perform two database queries to return the permission when checkingToken. The new version of token can return permission information without checking the library.
...
- Copy token to other environment verification fails
  ...
  + The client ua will be verified in uni-id. If you are debugging locally, you can modify `context.CLIENTUA` in the cloud function to be the device ua that generates the token. Remember to delete this logic when going online. If you do not need to bind the device and the token, you can configure `bindTokenToDevice: false` in the config to close the binding, `uni-id 3.0.12` and above versions adjust the default value of bindTokenToDevice to false
