::: warning 注意 
 您正在浏览的是老版uni统计1.0的文档，目前已自动升级到[uni统计2.0](uni-stat-public.md)，下方文档是过期文档。
:::
...
2. 自定义事件怎么用 
 使用 `uni.report()` API 上报数据，详见[自定义事件说明](api/other/report.md)

3. 内容统计是什么/ 页面规则怎么配置
 内容统计是uni统计的特色功能之一，是内容详情页的访问统计，详见[内容统计](uni-stat-content.md)
