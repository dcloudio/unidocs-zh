...
|Method|Parameter|Description|Platform Difference Description|Minimum Version|
|:-|:-|:-|:-|:-|
|getCenterLocation|OBJECT|获取当前地图中心的经纬度，返回的是 gcj02 坐标系，可以用于 [uni.openLocation](api/location/location?id=getlocation)|||
| getCenterLocation| OBJECT|Get the latitude and longitude of the current map center and return the gcj02 coordinate system, which can be used in [uni.openLocation](api/location/location?id=getlocation)|||
|moveToLocation|OBJECT|将地图中心移动到当前定位点，需要配合map组件的show-location使用|||
| moveToLocation| OBJECT|Move the center of the map to the current positioning point, it needs to be used with the show-location of the map component|||
|translateMarker|OBJECT|平移marker，带动画||app-nvue 2.1.5+、微信小程序带动画、字节、支付宝、京东、百度、QQ小程序|
| translateMarker| MiniApp|Translation marker with animation ||
|includePoints|OBJECT|缩放视野展示所有经纬度||app-nvue 2.1.5+、微信、字节、支付宝、京东、百度、快手、QQ小程序|
| Kuaishou| OBJECT|Zoom the field of view to display all latitude and longitude||
|getRegion|OBJECT|获取当前地图的视野范围|||
| getRegion| OBJECT|Get the field of view of the current map|||
|getRotate|OBJECT|获取当前地图的旋转角||微信、字节、支付宝、京东、QQ小程序|
| getRotate| OBJECT|Get the rotation angle of the current map||WeChat, Byte, Alipay, JD.com, QQ MiniApp|
|getScale|OBJECT|获取当前地图的缩放级别|||
| getScale| OBJECT|Get the zoom level of the current map|||
|getSkew|OBJECT|获取当前地图的倾斜角||微信、字节、支付宝、京东、QQ小程序|
| getSkew| OBJECT|Get the tilt angle of the current map||WeChat, Byte, Alipay, JD.com, QQ MiniApp|
|addCustomLayer|OBJECT|添加个性化图层||微信小程序|
| addCustomLayer| OBJECT|Add a personalized layer||WeChat MiniApp|
|addGroundOverlay|OBJECT|创建自定义图片图层，图片会随着地图缩放而缩放||App-nvue 3.1.0+，微信、字节小程序|
| addGroundOverlay| OBJECT|Create a custom image layer, the image will scale with the map zoom|| App-nvue 3.1.0+, WeChat, byte MiniApp|
|addMarkers |OBJECT|添加 marker||App-nvue 3.1.0+，微信小程序|
| addMarkers | OBJECT|Add marker|| App-nvue 3.1.0+, WeChat MiniApp|
|fromScreenLocation|OBJECT|获取屏幕上的点对应的经纬度，坐标原点为地图左上角||微信小程序|
| fromScreenLocation| OBJECT|Get the latitude and longitude corresponding to the point on the screen, the coordinate origin is the upper left corner of the map||WeChat MiniApp|
|initMarkerCluster|OBJECT|初始化点聚合的配置，未调用时采用默认配置||App-nvue 3.1.0+，微信小程序|
| initMarkerCluster| OBJECT|Configuration of initialization point aggregation, default configuration is used when not called|| App-nvue 3.1.0+, WeChat MiniApp|
...
| moveAlong| OBJECT|Move the marker along the specified path, which is used for track playback and other scenarios. The callback event is triggered when the animation is completed. If the animation is in progress, call the moveAlong method on the same marker again, and the previous animation will be interrupted. |Support android does not support autoRotate attribute setting default ture| App-nvue 3.1.0+, WeChat, byte MiniApp|
|openMapApp|OBJECT|拉起地图APP选择导航。||App-nvue 3.1.0+，微信、字节、快手小程序|
| openMapApp| OBJECT| Pull up the map app and choose navigation. || App-nvue 3.1.0+, WeChat, Byte, Kuaishou MiniApp|
|removeCustomLayer|OBJECT|移除个性化图层 ||微信小程序|
| removeCustomLayer| OBJECT|Remove personalized layer||WeChat MiniApp|
|removeGroundOverlay|OBJECT|移除自定义图片图层||App-nvue 3.1.0+，微信小程序|
| removeGroundOverlay| OBJECT|Remove custom image layer|| App-nvue 3.1.0+, WeChat MiniApp|
|removeMarkers|OBJECT|移除 marker。||App-nvue 3.1.0+，微信小程序|
| removeMarkers| OBJECT| Remove markers. || App-nvue 3.1.0+, WeChat MiniApp|
|setCenterOffset|OBJECT|设置地图中心点偏移，向后向下为增长，屏幕比例范围(0.25~0.75)，默认偏移为[0.5, 0.5]||微信、字节小程序|
| setCenterOffset| OBJECT|Set the offset of the center point of the map, backward and downward is growth, the screen scale range (0.25~0.75), the default offset is [0.5, 0.5]||WeChat, Byte MiniApp|
|toScreenLocation|OBJECT|获取经纬度对应的屏幕坐标，坐标原点为地图左上角。||微信小程序|
| toScreenLocation| OBJECT|Get the screen coordinates corresponding to the latitude and longitude. The origin of the coordinates is the upper left corner of the map. ||WeChat MiniApp|
|updateGroundOverlay|OBJECT|更新自定义图片图层。||App-nvue 3.1.0+，微信、字节小程序|
| updateGroundOverlay| OBJECT|Updates a custom image layer. || App-nvue 3.1.0+, WeChat, byte MiniApp|
|on|Method|监听地图事件。||App-nvue 3.1.0+，微信小程序|
| on| Method|Listen to map events. || App-nvue 3.1.0+, WeChat MiniApp|
|$getAppMap||获取原生地图对象 [plus.maps.Map](https://www.html5plus.org/doc/zh_cn/maps.html#plus.maps.Map)|app-vue|1.9.3|
|$getAppMap||Get native map object [plus.maps.Map](https://www.html5plus.org/doc/zh_cn/maps.html#plus.maps.Map)| app-vue| 1.9.3|
...
* The following is a brief introduction to the authorization and charges of some map service providers. The latest information published on the official website of the map service provider shall prevail.
  ...
  * Google Maps: Pay as you go, get some monthly credits.
