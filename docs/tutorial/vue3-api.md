...
When the `Vue` component is compiled to the applet platform, it will be compiled into the component of the corresponding platform. Some applet platforms support the `options` option (for specific options, please refer to the custom components section of the corresponding applet platform documentation). Generally, the default is sufficient. If you have special needs, you can add the `options` attribute to the `Vue` component.
