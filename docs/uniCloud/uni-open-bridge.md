...
3. The supporting database, save these credentials, the table name is [opendb-open-data](https://gitee.com/dcloud/opendb/tree/master/collection). The key format in redis is `uni-id:[dcloudAppid]:[platform]:[openid]:[access-token|user-access-token|session-key|encrypt-key-version|ticket]`
...
- `encrypt_key` depends on `access_token`, `session_key`, if the dependent value already exists, you can directly read `encrypt_key`, if it does not exist, it is automatically obtained from the WeChat server, the developer should only read this value, if there is an external system Writable when dependent
...
3. [access_token](#access_token), [session_key](#session_key), [encrypt_key](#encrypt_key) are required to decrypt the encrypted data used by the [uni cloud integrated security network]() channel
