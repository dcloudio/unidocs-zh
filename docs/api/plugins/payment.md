...
uni.requestPayment is a client-side payment API that unifies all platforms. Whether it is in a small program or an App, the client uses this API to call payment.
...
For example, WeChat has different application portals and usage processes, such as App Payment, Mini Program Payment, H5 Payment, etc. Corresponding to uni-app, you need to apply for WeChat App Payment on the App side, while the Mini Program side applies for WeChat Mini Program Payment.
...
If the server uses [uniCloud](https://uniapp.dcloud.io/uniCloud/README), then the official provides [uniPay](https://uniapp.dcloud.io/uniCloud/unipay) cloud unified payment service, The server-side payment development in the App, WeChat applet, and Alipay applet has been uniformly packaged.
...
|orderInfo|String/Object|Yes|Order data, [Notes](/api/plugins/payment?id=orderinfo)|App, Alipay applet, Baidu applet, ByteDance applet|
|timeStamp|String|微信小程序必填|时间戳从1970年1月1日至今的秒数，即当前的时间。|微信小程序|
|timeStamp|String|Required for WeChat Mini Programs|Timestamp seconds from January 1, 1970 to the present, that is, the current time. |WeChat Mini Program|
|nonceStr|String|微信小程序必填|随机字符串，长度为32个字符以下。|微信小程序|
|nonceStr|String|Required for WeChat Mini Programs|Random string with a length of less than 32 characters. |WeChat Mini Program|
|package|String|微信小程序必填|统一下单接口返回的 prepay_id 参数值，提交格式如：prepay_id=xx。|微信小程序|
|package|String|Required for WeChat Mini Program|The value of the prepay_id parameter returned by the unified order interface. The submission format is: prepay_id=xx. |WeChat Mini Program|
|signType|String|微信小程序必填|签名算法，应与后台下单时的值一致|微信小程序|
|signType|String|Required for WeChat Mini Program|Signature algorithm, which should be the same as the value when placing an order in the background|WeChat Mini Program|
...
|paySign|String|Required for WeChat Mini Program|Signature, please refer to [WeChat Mini Program Payment Document](https://pay.weixin.qq.com/wiki/doc/api/wxa/wxa_api.php?chapter for the specific signature scheme =7_7&index=3)|WeChat Mini Program|
...
|bannedChannels|Array&lt;String&gt;|No|A hidden payment method is required, see [Baidu Mini Program Payment Documentation](https://smartprogram.baidu.com/docs/develop/api/open_payment/#requestPolymerPayment/)|Baidu applet |
...
|service|Number|Required for the ByteDance applet|Fixed value: 1 (pull up the applet checkout desk) If the developer does not want to use the ByteDance applet checkout desk, when the service is set to 3/4, you can directly pull Start WeChat/Alipay for payment: service=3: WeChat API payment, without opening the mini program cashier; service=4: Alipay API payment, without opening the applet cashier. Where service=3, 4, only supported in 1.35.0.1+ basic library (Toutiao 743+)|Byte Beat applet|
|_debug|Number|否|仅限调试用，上线前去掉该参数。_debug=1时，微信支付期间可以看到中间报错信息，方便调试|字节跳动小程序|
|_debug|Number|No|For debugging only, remove this parameter before going online. When _debug=1, you can see the intermediate error message during WeChat payment, which is convenient for debugging|Byte Beat applet|
|getOrderStatus|Function|字节跳动小程序必填|商户前端实现的查询支付订单状态方法（该方法需要返回个Promise对象）。 service=3、4时不需要传。|字节跳动小程序|
|getOrderStatus|Function|Required for ByteDance Mini Program|The method of querying payment order status implemented by the merchant front-end (this method needs to return a Promise object). No need to pass when service=3 and 4. |ByteDance Mini Program|
...
微信小程序 `signType` 说明
WeChat applet `signType` description
...
- Please pay attention to [Initiate Toutiao Payment] (https://developer.toutiao.com/dev/cn/mini-app/develop/open-capacity/payment/pay) when adjusting and using the ByteDance applet payment interface
...
1. The orderInfo of Baidu Mini Program is of type Object. For detailed data structure, please refer to: [Baidu Cashier Payment](https://smartprogram.baidu.com/docs/develop/api/open_payment/#requestPolymerPayment/).
2. 支付宝小程序的 orderInfo(支付宝的规范为 tradeNO) 为 String 类型，表示支付宝交易号。
2. The orderInfo of the Alipay applet (the specification of Alipay is tradeNO) is of String type, indicating the Alipay transaction number.
3. 字节跳动小程序的 orderInfo 为 Object 类型，详见：[发起头条支付](https://developer.toutiao.com/dev/cn/mini-app/develop/open-capacity/payment/pay)
3. The orderInfo of the ByteDance applet is of type Object, see: [Initiate Toutiao Payment](https://developer.toutiao.com/dev/cn/mini-app/develop/open-capacity/payment/pay)
...
4. Since the new version of Toutiao payment interface requires a higher version, if the new version of payment interface is not supported, it will still correspond to the old version of payment interface. At this time, orderInfo corresponds to ByteDance applet data. For details, please refer to: [Toutiao payment old version interface](https ://developer.toutiao.com/dev/cn/mini-app/develop/open-capacity/payment/requestpayment-deprecated). Users can use tt.pay to determine whether the new version of the interface is supported. In addition, it should be noted that the ByteDance applet supports canIUse in the `1.35.0+` version of the basic library, and supports the new version of the payment interface tt.pay in the `1.19.4+` version of the basic library, so you should avoid using canIUse to judge whether it is a new version interface.
...
- You can also use WeChat to pay in ordinary browsers. This is called H5 payment in WeChat. This function is not open to ordinary developers. You need to apply for WeChat separately. [See details](https://pay.weixin.qq .com/wiki/doc/api/H5.php?chapter=15_1)
...
- Only WeChat applet payment is supported in WeChat applet. When applying for payment on [WeChat Merchant Platform](https://pay.weixin.qq.com), select the official account to pay.
...
- 支付宝小程序只支持支付宝支付。
- The Alipay applet only supports Alipay payment.
- 百度小程序为百度支付，其二次封装了度小满、支付宝、微信支付。
- Baidu Mini Program is Baidu Pay, which encapsulates Duxiaoman, Alipay and WeChat Pay.
...
2. In `manifest.json - App SDK Configuration`, select the required payment platform, currently there are WeChat Pay, Alipay, Apple In-App Payment (IAP), and WeChat Pay needs to fill in the AppID obtained from the WeChat open platform
...
### application process
  ...
  Note that WeChat's App payment, Mini Program payment, and H5 payment are different systems. WeChat Mini Program Payment When applying for payment on the [WeChat Merchant Platform](https://pay.weixin.qq.com), choose the official account to pay; you can also call up WeChat to pay in ordinary browsers, which is called H5 payment in WeChat , this function is not open to ordinary developers, you need to apply to WeChat separately, [see details](https://pay.weixin.qq.com/wiki/doc/api/H5.php?chapter=15_1)
...
3.5.1 The problem of losing orders in some cases due to automatic order closing

3.5.1 + 增加了手动关闭订单参数 `manualFinishTransaction`, 在合适的时机调用 `iapChannel.finishTransaction` 关闭订单
3.5.1 + Added manual close order parameter `manualFinishTransaction`, call `iapChannel.finishTransaction` at the right time to close the order

3.5.1+ 开始支持通过 `uni.getProvider` 获取IAP支付通道的方法
3.5.1+ Start to support the method of getting IAP payment channel through `uni.getProvider`
...
- Calling `finishTransaction` to close an order may not take effect immediately, depending on Apple's servers
    ...
    methods: {
      ...
      async restore() {
        ...
        try {
          ...
          // Developer business logic, get the current user's unfinished order list from the server, and compare it with the local
          // 此处省略
          // omitted here
...
- Front-end: use ``uni.request`` to request the server interface, get the order data, use ``uni.requestPayment`` to initiate a payment request to the payment platform, and pull up the payment platform's client to pay. Detailed code in hello uni-app.
...
- Q: How to use other payments on the App, such as UnionPay and PayPal.
  ...
  A: App 3.4+ already supports PayPal, the previous version of App 3.4 uses the following scheme
	...
	1. You can use their wap version for payment in the web-view component; 2. You can integrate native sdk, which are available in the plug-in market, [see details](https://ext.dcloud.net.cn/search?q=% E6%94%AF%E4%BB%98). You can also develop native plug-ins by yourself. For the development documents, see [https://ask.dcloud.net.cn/article/35428](https://ask.dcloud.net.cn/article/35428).
	1. You can use their wap version for payment in the web-view component; 2. You can integrate native sdk, which are available in the plug-in market, [see details](https://ext.dcloud.net.cn/search?q=% E6%94%AF%E4%BB%98). You can also develop native plug-ins by yourself. For the development documents, see [https://ask.dcloud.net.cn/article/35428](https://ask.dcloud.net.cn/article/35428).
	1. You can use their wap version for payment in the web-view component; 2. You can integrate native sdk, which are available in the plug-in market, [see details](https://ext.dcloud.net.cn/search?q=% E6%94%AF%E4%BB%98). You can also develop native plug-ins by yourself. For the development documents, see [https://ask.dcloud.net.cn/article/35428](https://ask.dcloud.net.cn/article/35428).
	1. You can use their wap version for payment in the web-view component; 2. You can integrate native sdk, which are available in the plug-in market, [see details](https://ext.dcloud.net.cn/search?q=% E6%94%AF%E4%BB%98). You can also develop native plug-ins by yourself. For the development documents, see [https://ask.dcloud.net.cn/article/35428](https://ask.dcloud.net.cn/article/35428).
