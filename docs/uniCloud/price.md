...
|读操作使用量	|1万RU/日	|20分钟				|
|写操作使用量	|1万WU/日	|20分钟				|
|云存储 #{rowspan=4}|容量			|1GB		|10小时				|
|下载操作次数	|1万次/日	|10小时				|
|上传操作次数	|1万次/日	|10小时				|
|CDN 流量		|1GB/日		|10小时				|
|前端网站托管 #{rowspan=2}|容量			|1GB		|10小时				|
|流量|1GB/日		|10小时				|

上述指标，容量是累计的，其他单位则是按日的。

例如，设置了每日云存储下载次数不能超过1万次。如某日该超过该下载次数，则在超过的10小时后停止云存储的下载功能。同时不影响其他资源指标的使用。
...
|云存储 #{rowspan=4}|容量（GB）			| -																																												|10小时				|
|下载操作次数		|通过CDN加速访问的次数，回源次数暂不收费。																																	|10小时				|
|上传操作次数		|  -																																											|10小时				|
|CDN 流量（GB）		|通过CDN加速产生的流量，回源流量暂不收费。																																		|10小时				|
|前端网站托管 #{rowspan=2}|容量（GB）			| -																																												|10小时				|
|CDN 流量（GB）		|通过CDN加速产生的流量，回源流量暂不收费。																																		|10小时				|
