...
---
### 激励视频开启了实时价格，回调里面没有cpm参数或cpm为0？
激励视频回调里面的cpm实际是基于bidding广告位的实时价格，如没有bidding广告位填充，则会触发广告位兜底，此时可能没有cpm参数，或cpm值为0。故激励视频回调里面的cpm参数并不是100%返回有效值，开发对接时需注意代码兼容处理（同时判断cpm是否存在以及是否大于0）。

---

### 优量汇状态显示`应用暂停` 原因：无广告请求 如何重新开启？
应用前日广告展示量大于1000后，优量汇审核状态会自动变为`审核驳回`，重新申请即可。

---

### 为什么应用关闭或广告位停用后，App 仍有广告曝光？

这是正常现象，原因在于缓存机制：
- **服务器缓存**：广告位停用配置生效通常有约 20 分钟的延迟。
- **客户端缓存**：App 客户端本地可能已预加载了一条广告数据，即使用户关闭了应用或广告位，这条已缓存的广告仍可能被展示一次。
