...
我们会**根据公测版服务空间资源用量和存储空间来推荐迁移正式版的套餐，低于套餐可能因为存储不足而迁移失败或刚迁移结束就欠费。所以只可购买推荐套餐及以上的版本以保证全量数据迁移**，您也可以直接选择按量计费。

因为老空间未提供流量统计，所以很多开发者不知道自己的流量和存储消耗多大，可能会惊讶于最低套餐的标准。

对于认为套餐不合理的开发者，推荐改用按量付费，跑一段时间看看使用量到底是多少，然后再决定是否改为套餐模式。
