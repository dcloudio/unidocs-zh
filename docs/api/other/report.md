::: warning 注意
推荐使用 [uni统计2.0](../../uni-stat.md)。开启方式见 [公有版](../../uni-stat-public.md) 或 [私有版](../../uni-stat-private.md)。
:::
...
|避免与内置事件重名|不要用 `login`、`share`、`pay_success`、`pay_fail`（见下文 [内置事件](#builtin-events)）|
...
适用场景：页面标题由接口动态下发，未调用 `uni.setNavigationBarTitle()`，但仍希望在内容统计报表中看到可读的名称。完整说明见 [内容统计](../../uni-stat-content.md#page-title)。
...
控制台出现 `[uni统计 2.0]` 前缀的日志，即表示调用已被统计模块接收。更多调试说明见 [公有版发行与调试](../../uni-stat-public.md#release-debug)。
...
A：uni统计2.0 公有版在微信小程序**无需**配置 request 合法域名。若使用其他小程序平台，须将 `tongji-collector.dcloud.net.cn` 加入 request 合法域名，详见 [公有版域名说明](../../uni-stat-public.md#mp-request-domain)。
