...
如果是运行到鸿蒙平台，流程与Android/iOS不同。HBuilderX会通过【运行到鸿蒙】入口调用鸿蒙工具链，生成或更新鸿蒙工程并安装`.hap`到鸿蒙真机或模拟器，不使用Android/iOS调试基座。
...
如果是运行到鸿蒙平台，请使用HBuilderX菜单【运行】-【运行到鸿蒙】。鸿蒙平台目前面向uni-app Vue3项目，需使用支持鸿蒙运行能力的HBuilderX版本，并先按[鸿蒙的运行和发行功能](./run-app-harmony.md)完成DevEco Studio配置。
...
运行到鸿蒙平台时，HBuilderX需要先找到鸿蒙工具链和可用设备。

1. 电脑需安装DevEco Studio，并在HBuilderX中正确配置`harmony.devTools.path`。请参考[鸿蒙的运行和发行功能](./run-app-harmony.md)手动配置。
2. 如果运行到鸿蒙真机，需要真机系统版本满足鸿蒙运行要求，并在手机上开启开发者模式和USB调试。连接电脑后，按手机端提示允许该电脑调试。
3. 如果运行到鸿蒙模拟器，需要先在DevEco Studio中创建并启动模拟器，待模拟器完全启动后再回到HBuilderX执行【运行到鸿蒙】。
4. 如果HBuilderX检测不到鸿蒙设备，优先确认DevEco Studio能否识别该真机或模拟器；如果DevEco Studio也识别不到，请先排查USB线、真机调试授权、模拟器启动状态和鸿蒙工具链安装是否正常。
...
鸿蒙平台运行流程不同：HBuilderX会编译uni-app项目，准备鸿蒙工程，调用鸿蒙工具链构建`.hap`，再安装并启动到选中的鸿蒙真机或模拟器。首次运行时，项目根目录可能会生成`harmony-configs`目录，用于存放鸿蒙工程配置。运行到鸿蒙真机通常需要配置调试签名；如遇包名、签名、权限、安装失败等问题，请参考[鸿蒙的运行和发行功能](./run-app-harmony.md)中的说明。
...
#### Q15: 运行到鸿蒙检测不到真机或模拟器怎么办？

1. 确认已安装DevEco Studio，并正确配置`harmony.devTools.path`。
2. 真机运行时，确认手机已开启开发者模式和USB调试，并已允许当前电脑调试。
3. 模拟器运行时，先在DevEco Studio中启动模拟器，等模拟器完全启动后再运行。
4. 如果DevEco Studio也检测不到设备，请先按鸿蒙开发工具链自身的设备连接问题排查。

#### Q16: 运行到鸿蒙真机提示没有签名、签名验证失败或签名不一致怎么办？

鸿蒙真机运行通常需要调试签名。请检查`harmony-configs/build-profile.json5`中的签名配置，确认签名证书、profile、设备UUID和项目包名匹配。如果提示签名不一致，可能是设备上已安装了使用其他签名的同包名应用，请先卸载旧应用后重新运行。新版本已经支持在`manifest.json`里面的【鸿蒙App配置】通过图形界面配置鸿蒙数字签名证书。
