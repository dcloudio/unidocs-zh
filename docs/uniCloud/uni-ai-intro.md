...
不同的模型在处理文本时分词方法不完全相同，可能存在特定的一句话在不同的模型里面会计算出完全不一样的token数量，此外服务商在处理输入时可能会加入一些额外token。如果服务商调整tokenizer算法也可能会导致token数计算变化。

对于连续对话，所有的前文及输出都会进行token数量的计算。如果使用uni-ai-chat还会在上下问过长时发送总结请求，也会额外消耗token数（如果不进行总结，多轮对话会因为token数太多无法进行）
...
gpt-3.5-turbo：输入token数23，输出token数226，总token数249
用户：写一段关于森林的100字小作文
